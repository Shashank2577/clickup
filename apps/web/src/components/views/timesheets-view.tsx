...
// === WIRING: fallback demo entries when API returns empty ===
const demoEntries: TimeEntry[] = [
  { id: 'demo-1', taskName: 'Implement user dashboard', location: 'Engineering / Sprint 12', daily: [0, 2, 3, 2, 4, 1, 0] },
  { id: 'demo-2', taskName: 'Code review PRs', location: 'Engineering / Sprint 12', daily: [0, 1, 1, 1, 1, 0, 0] },
  { id: 'demo-3', taskName: 'Team standup & planning', location: 'Engineering / Meetings', daily: [0, 0.5, 0.5, 0.5, 0.5, 0, 0] },
]
...
export function TimesheetsView() {
  ...
  // === WIRING: memoize weekStart string to stabilize useEffect dependency ===
  const weekStartStr = useMemo(() => start.toISOString().slice(0, 10), [start.getTime()])

  // === WIRING: fetch timesheet entries from API, re-fetch on week/tab change ===
  useEffect(() => {
    async function load() {
      setLoading(true)
      try {
        const data = await api.get<any[]>('/tasks/time-entries/timesheet', {
          params: {
            userId: activeTab === 'all-timesheets' ? undefined : 'me',
        ...
        const mapped = data.map((e) => ({
          ...
          taskName: e.taskName,
          location: e.location,
          daily: e.daily ?? [0, 0, 0, 0, 0, 0, 0],
        }))
        setEntries(mapped.length > 0 ? mapped : demoEntries)
      } catch {
  ...
  entries.forEach((e) => e.daily.forEach((v, i) => (totals[i] += v)))
  const grandTotal = totals.reduce((a, b) => a + b, 0)

  // === WIRING: add time entry for a task via API ===
  async function handleAddTask() {
    // In a full implementation, this would open a task picker modal.
    // For now, post a placeholder time entry.
    try {
      await api.post('/tasks/time-entries', {
        body: { duration: 3600 },
      })
    } catch {
      // Silently fail
            ...
            <thead>
              <tr className="border-b border-border">
                ...
                {dates.map((d, i) => (
                  <th
                    key={i}
                    className="px-3 py-2 text-center text-2xs font-medium uppercase tracking-wider min-w-[80px]"
                  >
                    <div>{dayLabels[i]}</div>
