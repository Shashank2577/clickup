...
async function seedComment(taskId: string, authorId: string, body: string): Promise<void> {
  await db.query(
  ...
  for (const spaceData of spaces) {
    ...
    for (const listName of listNames) {
      ...
      for (const task of taskData) {
        ...
        await seedComment(taskId, DEMO_USER_ID, `Initial notes for: ${task.title}`)
        await seedComment(taskId, DEMO_USER_ID, 'Progress update: working on it.')
      }
    }
