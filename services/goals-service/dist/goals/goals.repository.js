...
export const goalsRepository = {
    ...
    async getGoalsForWorkspace(workspaceId) {
        const result = await db.query(`
      SELECT
        g.*,
        json_agg(
    ...
    async createGoal(workspaceId, name, description, dueDate, ownerId, color) {
        const result = await db.query(`
    ...
    async updateGoal(goalId, name, description, dueDate, color) {
        const result = await db.query(`
      UPDATE goals
    ...
    async getTargetsForGoal(goalId) {
        const result = await db.query(`
      SELECT
        id,
        goal_id as "goalId",
    ...
    async getTarget(targetId, goalId) {
        const result = await db.query(`
      SELECT
        id,
        goal_id as "goalId",
    ...
    async createTarget(goalId, name, type, targetValue, currentValue, taskId) {
        const result = await db.query(`
    ...
    async updateTarget(targetId, goalId, name, currentValue, targetValue, taskId) {
        const result = await db.query(`
      UPDATE goal_targets
    ...
    async getTaskWithWorkspace(taskId) {
        const result = await db.query(`
