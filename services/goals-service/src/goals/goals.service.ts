import { GoalTarget, GOAL_EVENTS, GoalTargetType } from '@clickup/contracts'
import { AppError, publish, createServiceClient } from '@clickup/sdk'
import { ErrorCode } from '@clickup/contracts'
import { goalsRepository } from './goals.repository.js'

export function computeGoalProgress(targets: any[]): number {
...
export function computeTargetProgress(target: any): number {
  switch (target.type) {
    case 'boolean':
...
export const goalsService = {
  async verifyWorkspaceMembership(workspaceId: string, userId: string, traceId: string) {
  ...
  async createGoal(workspaceId: string, name: string, description: string | null, dueDate: string | null, color: string | null, userId: string, traceId: string) {
    ...
    await publish(GOAL_EVENTS.CREATED as any, { goalId: goal.id, workspaceId, createdBy: userId })
    return goal
  },

  async getGoal(goalId: string, userId: string, traceId: string) {
    const goal = await goalsRepository.getGoal(goalId)
    if (!goal) throw new AppError(ErrorCode.GOAL_NOT_FOUND)
    await this.verifyWorkspaceMembership(goal.workspace_id, userId, traceId)
    return goal
  },

  async getGoalsForWorkspace(workspaceId: string, userId: string, traceId: string) {
    await this.verifyWorkspaceMembership(workspaceId, userId, traceId)
    return goalsRepository.getGoalsForWorkspace(workspaceId)
  },

  async updateGoal(goalId: string, input: any, userId: string, traceId: string) {
  ...
  async deleteGoal(goalId: string, userId: string, traceId: string) {
    await this.getGoal(goalId, userId, traceId)
    await goalsRepository.deleteGoal(goalId)
  },

  async addTarget(goalId: string, input: any, userId: string, traceId: string) {
    await this.getGoal(goalId, userId, traceId)
  ...
  async updateTarget(goalId: string, targetId: string, input: any, userId: string, traceId: string) {
    await this.getGoal(goalId, userId, traceId)
  ...
  async updateTargetValueForTask(taskId: string, value: number) {
  ...
  async recomputeProgress(goalId: string) {
    ...
    await publish(GOAL_EVENTS.PROGRESS_UPDATED as any, { goalId, progress })
  }
}
