...
export function pinnedRouter(db: Pool): Router {
  ...
  router.post('/', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    ...
    await db.query(
      `INSERT INTO pinned_tasks (user_id, task_id, list_id)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, task_id) DO NOTHING`,
      [req.auth.userId, taskId, task.list_id],
    )
  ...
  router.delete('/', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    ...
    await db.query(
      'DELETE FROM pinned_tasks WHERE user_id = $1 AND task_id = $2',
      [req.auth.userId, taskId],
    )
...
export function listPinnedHandler(db: Pool): RequestHandler {
  const repository = new TasksRepository(db)
  const identityUrl = process.env['IDENTITY_SERVICE_URL'] || 'http://localhost:3001'
  ...
  return asyncHandler(async (req, res): Promise<void> => {
    ...
    const { rows } = await db.query(
      `SELECT pt.task_id, pt.pinned_at,
