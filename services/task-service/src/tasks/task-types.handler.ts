...
export function taskTypesRouter(db: Pool): Router {
  ...
  router.get('/:workspaceId/task-types', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    const { workspaceId } = req.params as { workspaceId: string }
    await verifyMembership(workspaceId, req.auth.userId)

    const { rows } = await db.query(
      'SELECT * FROM task_types WHERE workspace_id = $1 ORDER BY position ASC, created_at ASC',
      [workspaceId],
    )
  ...
  router.post('/:workspaceId/task-types', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    const { workspaceId } = req.params as { workspaceId: string }
    await verifyMembership(workspaceId, req.auth.userId)

    const { name, color, icon, position, isDefault } = req.body as {
      name: string; color?: string; icon?: string; position?: number; isDefault?: boolean
    }
    ...
    const { rows: posRows } = await db.query(
      'SELECT COALESCE(MAX(position), -1) + 1 AS next FROM task_types WHERE workspace_id = $1',
      [workspaceId],
    )
    ...
    try {
      const { rows } = await db.query(
        `INSERT INTO task_types (workspace_id, name, color, icon, position, is_default)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [workspaceId, name, color ?? '#6366f1', icon ?? null, nextPosition, isDefault ?? false],
      )
      res.status(201).json({ data: toDto(rows[0]) })
    } catch (err: any) {
      if (err.code === '23505') throw new AppError(ErrorCode.TASK_TYPE_ALREADY_EXISTS)
      throw err
    }
  }))

  // PATCH /workspaces/:workspaceId/task-types/:id
  router.patch('/:workspaceId/task-types/:id', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    const { workspaceId, id } = req.params as { workspaceId: string; id: string }
    await verifyMembership(workspaceId, req.auth.userId)

    // Check exists
    const { rows: existing } = await db.query(
      'SELECT * FROM task_types WHERE id = $1 AND workspace_id = $2',
      [id, workspaceId],
    )
    if (!existing[0]) throw new AppError(ErrorCode.TASK_TYPE_NOT_FOUND)
    ...
    try {
      const { rows } = await db.query(
        `UPDATE task_types SET ${setClause} WHERE id = $1 AND workspace_id = $2 RETURNING *`,
        [id, workspaceId, ...Object.values(updates)],
      )
  ...
  router.delete('/:workspaceId/task-types/:id', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    const { workspaceId, id } = req.params as { workspaceId: string; id: string }
    await verifyMembership(workspaceId, req.auth.userId)

    const { rowCount } = await db.query(
      'DELETE FROM task_types WHERE id = $1 AND workspace_id = $2',
      [id, workspaceId],
    )
    if (!rowCount) throw new AppError(ErrorCode.TASK_TYPE_NOT_FOUND)
