...
export function createTaskHandler(db: Pool) {
  const service = createTasksService(db)
  return asyncHandler(async (req: Request, res: Response) => {
    const input = validate(CreateTaskSchema, req.body) as any
    const task = await service.createTask(req.auth!.userId, input, req.headers['x-trace-id'] as string)
    res.status(201).json({ data: task })
  })
}

export function getTaskHandler(db: Pool) {
  const service = createTasksService(db)
  return asyncHandler(async (req: Request, res: Response) => {
    const { taskId } = req.params
    const task = await service.getTask(req.auth!.userId, taskId!, req.headers['x-trace-id'] as string)
    res.json({ data: task })
  })
}

export function listTasksHandler(db: Pool) {
  const service = createTasksService(db)
  return asyncHandler(async (req: Request, res: Response) => {
    const { listId } = req.params
    const query = validate(TaskListQuerySchema, req.query) as any
...
export function updateTaskHandler(db: Pool) {
  const service = createTasksService(db)
  return asyncHandler(async (req: Request, res: Response) => {
    const { taskId } = req.params
    const updates = validate(UpdateTaskSchema, req.body) as any
    const task = await service.updateTask(req.auth!.userId, taskId!, updates, req.headers['x-trace-id'] as string)
    res.json({ data: task })
  })
}

export function deleteTaskHandler(db: Pool) {
  const service = createTasksService(db)
  return asyncHandler(async (req: Request, res: Response) => {
    const { taskId } = req.params
    await service.deleteTask(req.auth!.userId, taskId!, req.headers['x-trace-id'] as string)
    res.status(204).end()
  })
}
...
export function archiveTaskHandler(db: Pool) {
  return asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!meta) throw new AppError(ErrorCode.TASK_NOT_FOUND)
    ...
    await db.query(
      `UPDATE tasks SET status = 'archived', completed_at = NOW(), updated_at = NOW() WHERE id = $1`,
      [taskId],
    )
...
export function unarchiveTaskHandler(db: Pool) {
  return asyncHandler(async (req: Request, res: Response) => {
    const { taskId } = req.params as { taskId: string }
    const repository = new TasksRepository(db)
    const task = await repository.getTask(taskId)
    ...
    await db.query(
      `UPDATE tasks SET status = 'todo', completed_at = NULL, updated_at = NOW() WHERE id = $1`,
      [taskId],
    )
