...
export function taskTimeEntriesRouter(db: Pool): Router {
  ...
  router.get('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    const total = entries.reduce((sum: number, e: any) => sum + e.minutes, 0)
  ...
  router.post('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (ended <= started) throw new AppError(ErrorCode.TIME_ENTRY_INVALID_RANGE)

    const minutes = Math.round((ended.getTime() - started.getTime()) / 60_000)
    if (minutes <= 0) throw new AppError(ErrorCode.TIME_ENTRY_INVALID_RANGE)
  ...
  router.post('/timer/start', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    const { taskId } = req.params as { taskId: string }
    const task = await repository.getTask(taskId)
    if (!task) throw new AppError(ErrorCode.TASK_NOT_FOUND)
    ...
    const { rows: active } = await db.query(
      'SELECT id FROM time_entries WHERE task_id = $1 AND user_id = $2 AND ended_at IS NULL',
    ...
    const { rows } = await db.query(
      'INSERT INTO time_entries (id, task_id, user_id, started_at, billable) VALUES ($1, $2, $3, NOW(), $4) RETURNING *',
      [id, taskId, req.auth.userId, isBillable],
    )
  ...
  router.post('/timer/stop', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    const { taskId } = req.params as { taskId: string }
    const task = await repository.getTask(taskId)
    if (!task) throw new AppError(ErrorCode.TASK_NOT_FOUND)
    ...
    const { rows: timers } = await db.query(
      'SELECT * FROM time_entries WHERE task_id = $1 AND user_id = $2 AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1',
      [taskId, req.auth.userId],
    )
    if (timers.length === 0) {
    ...
    const { rows } = await db.query(
      'UPDATE time_entries SET ended_at = NOW(), minutes = $1 WHERE id = $2 RETURNING *',
...
export function timeEntriesRouter(db: Pool): Router {
  const router = Router()
  const repository = new TasksRepository(db)
  ...
  router.get('/timesheet', requireAuth, asyncHandler(async (req, res): Promise<void> => {
    const { workspaceId, from, to, userId: filterUserId, groupBy } = req.query as Record<string, string>

    if (!workspaceId) throw new AppError(ErrorCode.VALIDATION_MISSING_FIELD, 'workspaceId is required')
    if (!from || !to) throw new AppError(ErrorCode.VALIDATION_MISSING_FIELD, 'from and to are required')
    ...
    const params: unknown[] = [workspaceId, fromDate, toDate]
  ...
  router.patch('/:entryId', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (entry.user_id !== req.auth.userId) throw new AppError(ErrorCode.AUTH_INSUFFICIENT_PERMISSION)

    const input = validate(UpdateTimeEntrySchema, req.body)
    const updates: Record<string, unknown> = {}

    if (input.startedAt !== undefined) updates['started_at'] = new Date(input.startedAt)
    ...
    if (new Date(newEnd as string) <= new Date(newStart as string)) {
      throw new AppError(ErrorCode.TIME_ENTRY_INVALID_RANGE)
    }
    updates['minutes'] = Math.round(
      (new Date(newEnd as string).getTime() - new Date(newStart as string).getTime()) / 60_000,
    )
