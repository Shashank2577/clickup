...
export function recurringRouter(db: Pool): Router {
  ...
  router.get('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    const { rows } = await db.query(
      'SELECT * FROM recurring_task_configs WHERE task_id = $1',
      [taskId],
    )
    if (!rows.length) throw new AppError(ErrorCode.RECURRING_CONFIG_NOT_FOUND)
  ...
  router.put('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    const { rows } = await db.query(
      `INSERT INTO recurring_task_configs (task_id, cron_expr, next_run_at, is_active)
       VALUES ($1,$2,$3,$4)
       ...
       RETURNING *`,
      [taskId, cronExpr, nextRunAt, input.isActive],
    )
  ...
  router.patch('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!existing.length) throw new AppError(ErrorCode.RECURRING_CONFIG_NOT_FOUND)

    const input = validate(UpdateRecurringConfigSchema, req.body)
    const updates: Record<string, unknown> = {}
    if (input.isActive !== undefined) updates['is_active'] = input.isActive
    if (input.cronExpr !== undefined) updates['cron_expr'] = input.cronExpr
    else if (input.frequency !== undefined) updates['cron_expr'] = FREQUENCY_CRON_MAP[input.frequency]

    const fields = Object.keys(updates)
    if (!fields.length) { res.json({ data: toConfigDto(existing[0]) }); return }

    const setClause = fields.map((f, i) => `${f} = $${i + 2}`).join(', ')
    const { rows } = await db.query(
      `UPDATE recurring_task_configs SET ${setClause} WHERE task_id = $1 RETURNING *`,
  ...
  router.delete('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    const { rowCount } = await db.query(
      'DELETE FROM recurring_task_configs WHERE task_id = $1',
      [taskId],
    )
    if (!rowCount) throw new AppError(ErrorCode.RECURRING_CONFIG_NOT_FOUND)
    res.status(204).end()
  }))
...
export function startRecurringTaskRunner(db: Pool): void {
  const run = async () => {
    try {
      // Get all active configs due to run
      const { rows } = await db.query(
        `SELECT rtc.*, t.list_id, t.title, t.description, t.priority, t.assignee_id
      ...
      for (const config of rows) {
        ...
        await db.query(
          `INSERT INTO tasks (id, list_id, path, title, description, priority, assignee_id, created_by, version)
           VALUES ($1,$2,$3,$4,$5,$6,$7,NULL,0)`,
          [newId, config.list_id, newId, config.title, config.description, config.priority, config.assignee_id],
        )
        ...
        await db.query(
          'UPDATE recurring_task_configs SET last_run_at = NOW(), next_run_at = $2 WHERE id = $1',
          [config.id, next],
        )
      }
    } catch (err) {
