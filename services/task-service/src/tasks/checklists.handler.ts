...
function toChecklistDto(row: any) {
  return {
    id: row.id,
    taskId: row.task_id,
    title: row.title,
...
export function checklistsRouter(db: Pool): Router {
  ...
  router.post('/:checklistId/items', requireAuth, asyncHandler(async (req, res) => {
    const { taskId, checklistId } = req.params as { taskId: string; checklistId: string }
    const task = await repository.getTask(taskId)
    if (!task) throw new AppError(ErrorCode.TASK_NOT_FOUND)

    const checklist = await repository.getChecklist(checklistId)
    if (!checklist || checklist.task_id !== taskId) throw new AppError(ErrorCode.CHECKLIST_NOT_FOUND)
...
export function checklistItemsRouter(db: Pool): Router {
  const router = Router()
  const repository = new TasksRepository(db)
  ...
  router.delete('/:checklistId', requireAuth, asyncHandler(async (req, res) => {
    ...
    const checklist = await repository.getChecklist(checklistId)
    if (!checklist) throw new AppError(ErrorCode.CHECKLIST_NOT_FOUND)
...
export function checklistItemRouter(db: Pool): Router {
  const router = Router()
  const repository = new TasksRepository(db)
  ...
  router.patch('/:itemId', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!item) throw new AppError(ErrorCode.CHECKLIST_ITEM_NOT_FOUND)
  ...
  router.post('/:itemId/convert', requireAuth, asyncHandler(async (req, res) => {
    const { itemId } = req.params as { itemId: string }
    const { asSubtask } = req.query as { asSubtask?: string }
    ...
    if (!parentTask) throw new AppError(ErrorCode.TASK_NOT_FOUND)

    const listId: string = parentTask.list_id
    const newId = randomUUID()
    const parentId: string | null = asSubtask === 'true' ? checklist.task_id : null
