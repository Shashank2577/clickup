...
function toRelationDto(row: any) {
  return {
    id: row.id,
    taskId: row.task_id,
...
export function relationsRouter(db: Pool): Router {
  ...
  router.post('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (taskId === input.relatedTaskId) throw new AppError(ErrorCode.TASK_SELF_RELATION)

    const task = await repository.getTask(taskId)
    if (!task) throw new AppError(ErrorCode.TASK_NOT_FOUND)

    const relatedTask = await repository.getTask(input.relatedTaskId)
    if (!relatedTask) throw new AppError(ErrorCode.TASK_NOT_FOUND)
    ...
    try {
      ...
      if (err.code === '23505') throw new AppError(ErrorCode.TASK_RELATION_ALREADY_EXISTS)
      throw err
    }
  }))
  ...
  router.delete('/:relationId', requireAuth, asyncHandler(async (req, res) => {
    const { taskId, relationId } = req.params as { taskId: string; relationId: string }
    const task = await repository.getTask(taskId)
    if (!task) throw new AppError(ErrorCode.TASK_NOT_FOUND)
