...
export function reorderRouter(db) {
    ...
    router.patch('/:listId/tasks/reorder', requireAuth, asyncHandler(async (req, res) => {
       ...
       SET position = CASE ${caseLines} END,
           updated_at = NOW(),
           version = version + 1
       WHERE list_id = $1 AND id = ANY(ARRAY[${idPlaceholders}]::uuid[]) AND deleted_at IS NULL`, [listId, ...taskIds]);
        res.json({ data: { reordered: taskIds.length } });
    }));
    // PATCH /lists/:listId/statuses/reorder
    router.patch('/:listId/statuses/reorder', requireAuth, asyncHandler(async (req, res) => {
        const { listId } = req.params;
        ...
        await db.query(`UPDATE list_statuses
       SET position = CASE ${caseLines} END
       WHERE list_id = $1 AND id = ANY(ARRAY[${idPlaceholders}]::uuid[])`, [listId, ...statusIds]);
        res.json({ data: { reordered: statusIds.length } });
    }));
    return router;
}
//# sourceMappingURL=reorder.handler.js.map