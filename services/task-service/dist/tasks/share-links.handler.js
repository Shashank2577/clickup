...
export function shareLinksRouter(db) {
    ...
    router.post('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const { rows } = await db.query(`INSERT INTO task_public_links (task_id, created_by)
       VALUES ($1, $2)
       ON CONFLICT (task_id) DO UPDATE SET is_active = TRUE
       RETURNING *`, [taskId, req.auth.userId]);
        const link = rows[0];
    ...
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!meta)
            throw new AppError(ErrorCode.LIST_NOT_FOUND);
        await verifyMembership(meta.workspace_id, req.auth.userId);
        const { rows } = await db.query('SELECT * FROM task_public_links WHERE task_id = $1', [taskId]);
        if (!rows[0])
            throw new AppError(ErrorCode.TASK_SHARE_LINK_NOT_FOUND);
        const link = rows[0];
    ...
    router.delete('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!meta)
            throw new AppError(ErrorCode.LIST_NOT_FOUND);
        await verifyMembership(meta.workspace_id, req.auth.userId);
        const { rowCount } = await db.query('UPDATE task_public_links SET is_active = FALSE WHERE task_id = $1', [taskId]);
        if (!rowCount)
            throw new AppError(ErrorCode.TASK_SHARE_LINK_NOT_FOUND);
        res.status(204).end();
    }));
...
export function publicShareHandler(db) {
    return asyncHandler(async (req, res) => {
        const { token } = req.params;
        const { rows } = await db.query(`SELECT tpl.*, t.id AS t_id, t.title, t.description, t.status, t.priority,
       ...
       WHERE tpl.token = $1 AND tpl.is_active = TRUE`, [token]);
        if (!rows[0])
            throw new AppError(ErrorCode.TASK_SHARE_LINK_NOT_FOUND);
        const row = rows[0];
        // Check expiry if set
...
//# sourceMappingURL=share-links.handler.js.map