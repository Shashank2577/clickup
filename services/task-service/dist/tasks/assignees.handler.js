...
export function assigneesRouter(db) {
    ...
    async function getAssignees(taskId) {
        const { rows } = await db.query(`SELECT ta.task_id, ta.user_id, ta.assigned_at, ta.assigned_by,
              u.name, u.avatar_url
       FROM task_assignees ta
       JOIN users u ON u.id = ta.user_id
       WHERE ta.task_id = $1
       ORDER BY ta.assigned_at ASC`, [taskId]);
        return rows;
    }
    ...
    router.post('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!meta)
            throw new AppError(ErrorCode.LIST_NOT_FOUND);
        await verifyMembership(meta.workspace_id, req.auth.userId);
        await db.query(`INSERT INTO task_assignees (task_id, user_id, assigned_by)
       VALUES ($1, $2, $3)
       ON CONFLICT (task_id, user_id) DO NOTHING`, [taskId, userId, req.auth.userId]);
        const assignees = await getAssignees(taskId);
    ...
    router.delete('/:userId', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!meta)
            throw new AppError(ErrorCode.LIST_NOT_FOUND);
        await verifyMembership(meta.workspace_id, req.auth.userId);
        await db.query('DELETE FROM task_assignees WHERE task_id = $1 AND user_id = $2', [taskId, userId]);
        res.status(204).end();
    }));
    return router;
}
//# sourceMappingURL=assignees.handler.js.map