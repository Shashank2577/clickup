...
export function statusesRouter(db) {
    const router = Router({ mergeParams: true });
    // GET /lists/:listId/statuses
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        const { listId } = req.params;
        const { rows } = await db.query('SELECT * FROM list_statuses WHERE list_id = $1 ORDER BY position ASC', [listId]);
    ...
    router.post('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        // Get next position
        const { rows: posRows } = await db.query('SELECT COALESCE(MAX(position), -1) + 1 AS next FROM list_statuses WHERE list_id = $1', [listId]);
        const position = input.position ?? posRows[0].next;
        try {
            const { rows } = await db.query('INSERT INTO list_statuses (list_id, name, color, position, is_closed) VALUES ($1,$2,$3,$4,$5) RETURNING *', [listId, input.name, input.color, position, input.isClosed]);
            res.status(201).json({ data: toStatusDto(rows[0]) });
        }
        catch (err) {
            if (err.code === '23505')
                throw new AppError(ErrorCode.LIST_STATUS_ALREADY_EXISTS);
            throw err;
        }
    }));
    ...
    router.patch('/:statusId', requireAuth, asyncHandler(async (req, res) => {
        ...
        const { rows: existing } = await db.query('SELECT * FROM list_statuses WHERE id = $1 AND list_id = $2', [statusId, listId]);
        if (!existing.length)
            throw new AppError(ErrorCode.LIST_STATUS_NOT_FOUND);
        ...
        if (!fields.length) {
            res.json({ data: toStatusDto(existing[0]) });
            return;
        }
        const setClause = fields.map((f, i) => `${f} = $${i + 3}`).join(', ');
        const { rows } = await db.query(`UPDATE list_statuses SET ${setClause} WHERE id = $1 AND list_id = $2 RETURNING *`, [statusId, listId, ...Object.values(updates)]);
    ...
    router.delete('/:statusId', requireAuth, asyncHandler(async (req, res) => {
        const { listId, statusId } = req.params;
        const { rowCount } = await db.query('DELETE FROM list_statuses WHERE id = $1 AND list_id = $2', [statusId, listId]);
        if (!rowCount)
            throw new AppError(ErrorCode.LIST_STATUS_NOT_FOUND);
        res.status(204).end();
    }));
...
//# sourceMappingURL=statuses.handler.js.map