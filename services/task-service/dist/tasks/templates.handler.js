...
export function taskTemplatesRouter(db) {
    const router = Router({ mergeParams: true });
    // GET /workspaces/:workspaceId/task-templates
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        const { workspaceId } = req.params;
        const { rows } = await db.query('SELECT * FROM task_templates WHERE workspace_id = $1 ORDER BY created_at DESC', [workspaceId]);
    ...
    router.post('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const { rows } = await db.query('INSERT INTO task_templates (workspace_id, name, description, template_data, created_by) VALUES ($1,$2,$3,$4,$5) RETURNING *', [workspaceId, input.name, input.description ?? null, JSON.stringify(input.templateData), req.auth.userId]);
    ...
    router.get('/:templateId', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!rows.length)
            throw new AppError(ErrorCode.TASK_TEMPLATE_NOT_FOUND);
    ...
    router.patch('/:templateId', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (input.templateData !== undefined)
            updates['template_data'] = JSON.stringify(input.templateData);
        const fields = Object.keys(updates);
        if (!fields.length) {
            res.json({ data: toTemplateDto(existing[0]) });
            return;
        }
        const setClause = fields.map((f, i) => `${f} = $${i + 2}`).join(', ');
        const { rows } = await db.query(`UPDATE task_templates SET ${setClause}, updated_at = NOW() WHERE id = $1 RETURNING *`, [templateId, ...Object.values(updates)]);
        res.json({ data: toTemplateDto(rows[0]) });
    }));
    ...
    router.post('/:templateId/use', requireAuth, asyncHandler(async (req, res) => {
        ...
        const { listId, parentId } = req.body;
        if (!listId)
            throw new AppError(ErrorCode.LIST_NOT_FOUND);
        const taskId = randomUUID();
        const td = template.template_data;
        const taskTitle = req.body.title ?? td.title ?? 'New Task';
        const path = parentId ? `${parentId}.${taskId}` : taskId;
        const { rows: taskRows } = await db.query(`INSERT INTO tasks (id, list_id, path, title, description, priority, parent_id, created_by, version)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,0) RETURNING *`, [taskId, listId, path, taskTitle, td.description ?? null, td.priority ?? 'none', parentId ?? null, req.auth.userId]);
        res.status(201).json({ data: taskRows[0] });
    }));
    return router;
}
//# sourceMappingURL=templates.handler.js.map