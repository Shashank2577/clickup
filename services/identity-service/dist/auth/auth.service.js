...
export class AuthService {
    ...
    async register(input) {
        ...
        const ctx = { userId: user.id, workspaceId: '', role: 'member', sessionId };
    ...
    async resetPassword(input) {
        ...
        if (record.used_at)
            throw new AppError(ErrorCode.AUTH_RESET_TOKEN_INVALID, 'Token already used');
        if (record.expires_at < new Date())
            throw new AppError(ErrorCode.AUTH_RESET_TOKEN_EXPIRED);
    ...
    async verifyEmail(input) {
        ...
        if (!record)
            throw new AppError(ErrorCode.AUTH_VERIFY_TOKEN_INVALID);
        if (record.verified_at)
            throw new AppError(ErrorCode.USER_EMAIL_ALREADY_VERIFIED);
        if (record.expires_at < new Date())
            throw new AppError(ErrorCode.AUTH_VERIFY_TOKEN_INVALID, 'Token expired');
