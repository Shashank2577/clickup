...
function defaultClickApps(workspaceId) {
    return {
        workspaceId,
        sprintsEnabled: true,
...
// ============================================================
export function userPreferencesRouter(db) {
    const router = Router();
    // GET /users/preferences — get current user's preferences
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        const r = await db.query(`SELECT user_id, theme, language, timezone, date_format, time_format,
                first_day_of_week, sidebar_collapsed, density, extra, updated_at
         FROM user_preferences WHERE user_id = $1`, [req.auth.userId]);
        const row = r.rows[0];
        if (!row) {
    ...
    router.put('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (input.timeFormat !== undefined) {
            setClauses.push(`time_format = $${idx++}`);
            values.push(input.timeFormat);
        }
        if (input.firstDayOfWeek !== undefined) {
            setClauses.push(`first_day_of_week = $${idx++}`);
            values.push(input.firstDayOfWeek);
        }
        if (input.sidebarCollapsed !== undefined) {
            setClauses.push(`sidebar_collapsed = $${idx++}`);
            values.push(input.sidebarCollapsed);
        }
        if (input.density !== undefined) {
            setClauses.push(`density = $${idx++}`);
            values.push(input.density);
        }
        if (input.extra !== undefined) {
            setClauses.push(`extra = $${idx++}`);
            values.push(JSON.stringify(input.extra));
        }
        const r = await db.query(`INSERT INTO user_preferences (user_id)
         VALUES ($1)
         ON CONFLICT (user_id) DO UPDATE SET ${setClauses.join(', ')}
         RETURNING user_id, theme, language, timezone, date_format, time_format,
                   first_day_of_week, sidebar_collapsed, density, extra, updated_at`, values);
...
// ============================================================
export function workspaceClickAppsRouter(db) {
    ...
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const r = await db.query(`SELECT workspace_id, sprints_enabled, time_tracking_enabled, priorities_enabled,
         ...
         FROM workspace_clickapps WHERE workspace_id = $1`, [workspaceId]);
        if (!r.rows[0]) {
    ...
    router.put('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const memberR = await db.query(`SELECT role FROM workspace_members WHERE workspace_id = $1 AND user_id = $2`, [workspaceId, req.auth.userId]);
        ...
        let idx = 2;
        if (input.sprintsEnabled !== undefined) {
            setClauses.push(`sprints_enabled = $${idx++}`);
            values.push(input.sprintsEnabled);
        }
        if (input.timeTrackingEnabled !== undefined) {
            setClauses.push(`time_tracking_enabled = $${idx++}`);
            values.push(input.timeTrackingEnabled);
        }
        if (input.prioritiesEnabled !== undefined) {
            setClauses.push(`priorities_enabled = $${idx++}`);
            values.push(input.prioritiesEnabled);
        }
        if (input.tagsEnabled !== undefined) {
            setClauses.push(`tags_enabled = $${idx++}`);
            values.push(input.tagsEnabled);
        }
        if (input.customFieldsEnabled !== undefined) {
            setClauses.push(`custom_fields_enabled = $${idx++}`);
            values.push(input.customFieldsEnabled);
        }
        if (input.automationsEnabled !== undefined) {
            setClauses.push(`automations_enabled = $${idx++}`);
            values.push(input.automationsEnabled);
        }
        if (input.goalsEnabled !== undefined) {
            setClauses.push(`goals_enabled = $${idx++}`);
            values.push(input.goalsEnabled);
        }
        if (input.aiEnabled !== undefined) {
            setClauses.push(`ai_enabled = $${idx++}`);
            values.push(input.aiEnabled);
        }
        if (input.milestonesEnabled !== undefined) {
            setClauses.push(`milestones_enabled = $${idx++}`);
            values.push(input.milestonesEnabled);
        }
        if (input.mindMapsEnabled !== undefined) {
            setClauses.push(`mind_maps_enabled = $${idx++}`);
            values.push(input.mindMapsEnabled);
        }
        if (input.whiteboardsEnabled !== undefined) {
            setClauses.push(`whiteboards_enabled = $${idx++}`);
            values.push(input.whiteboardsEnabled);
        }
        if (input.portfoliosEnabled !== undefined) {
            setClauses.push(`portfolios_enabled = $${idx++}`);
            values.push(input.portfoliosEnabled);
        }
        const r = await db.query(`INSERT INTO workspace_clickapps (workspace_id)
         VALUES ($1)
         ...
         RETURNING workspace_id, sprints_enabled, time_tracking_enabled, priorities_enabled,
                   tags_enabled, custom_fields_enabled, automations_enabled, goals_enabled,
                   ai_enabled, milestones_enabled, mind_maps_enabled, whiteboards_enabled,
                   portfolios_enabled, updated_at`, values);
        res.json({ data: toClickAppsDto(r.rows[0]) });
    }));
    return router;
}
