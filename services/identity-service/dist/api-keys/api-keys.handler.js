...
export function apiKeysRouter(db) {
    ...
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const r = await db.query(`SELECT id, workspace_id, user_id, name, key_hash, key_prefix, scopes,
                last_used_at, expires_at, created_at
         FROM api_keys
         WHERE workspace_id = $1 AND user_id = $2
         ORDER BY created_at DESC`, [workspaceId, req.auth.userId]);
    ...
    router.post('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const r = await db.query(`INSERT INTO api_keys (workspace_id, user_id, name, key_hash, key_prefix, scopes, expires_at)
    ...
    router.patch('/:id', requireAuth, asyncHandler(async (req, res) => {
        ...
        const r = await db.query(`UPDATE api_keys SET name = $1
         WHERE id = $2 AND user_id = $3 AND workspace_id = $4
         RETURNING id, workspace_id, user_id, name, key_hash, key_prefix, scopes,
                   last_used_at, expires_at, created_at`, [input.name, id, req.auth.userId, workspaceId]);
        if (r.rowCount === 0)
            throw new AppError(ErrorCode.API_KEY_NOT_FOUND);
    ...
    router.delete('/:id', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!memberR.rows[0])
            throw new AppError(ErrorCode.AUTH_WORKSPACE_ACCESS_DENIED);
        const r = await db.query(`DELETE FROM api_keys WHERE id = $1 AND user_id = $2 AND workspace_id = $3`, [id, req.auth.userId, workspaceId]);
        if (r.rowCount === 0)
            throw new AppError(ErrorCode.API_KEY_NOT_FOUND);
        res.status(204).end();
    }));
...
//# sourceMappingURL=api-keys.handler.js.map