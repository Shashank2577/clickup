...
export function recentlyViewedRouter(db) {
    ...
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const r = await db.query(`SELECT id, user_id, workspace_id, item_type, item_id, item_name, viewed_at
         FROM recently_viewed
         WHERE user_id = $1 AND workspace_id = $2
         ORDER BY viewed_at DESC
         LIMIT 20`, [req.auth.userId, workspaceId]);
    ...
    router.post('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const r = await db.query(`INSERT INTO recently_viewed (user_id, workspace_id, item_type, item_id, item_name, viewed_at)
         VALUES ($1, $2, $3, $4, $5, NOW())
         ON CONFLICT (user_id, item_type, item_id)
         DO UPDATE SET viewed_at = NOW(), item_name = EXCLUDED.item_name
         RETURNING id, user_id, workspace_id, item_type, item_id, item_name, viewed_at`, [req.auth.userId, workspaceId, input.itemType, input.itemId, input.itemName]);
        res.json({ data: toDto(r.rows[0]) });
    }));
    return router;
}
//# sourceMappingURL=recently-viewed.handler.js.map