...
export function workspaceInvitesRouter(db) {
    ...
    router.get('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        const r = await db.query(`SELECT id, workspace_id, email, role, token, invited_by, accepted_at, expires_at, created_at
    ...
    router.post('/', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!workspaceId)
            throw new AppError(ErrorCode.VALIDATION_INVALID_INPUT, 'workspaceId is required');
        const callerR = await db.query(`SELECT role FROM workspace_members WHERE workspace_id = $1 AND user_id = $2`, [workspaceId, req.auth.userId]);
        ...
        if (existingUser.rows[0]) {
            const existingMember = await db.query(`SELECT 1 FROM workspace_members WHERE workspace_id = $1 AND user_id = $2`, [workspaceId, existingUser.rows[0].id]);
            if (existingMember.rows[0])
                throw new AppError(ErrorCode.WORKSPACE_MEMBER_ALREADY_EXISTS);
        }
        // Upsert invite (workspace+email unique constraint)
        const r = await db.query(`INSERT INTO workspace_invites (workspace_id, email, role, invited_by)
         VALUES ($1, $2, $3, $4)
         ...
         RETURNING id, workspace_id, email, role, token, invited_by, accepted_at, expires_at, created_at`, [workspaceId, input.email, input.role, req.auth.userId]);
    ...
    router.delete('/:id', requireAuth, asyncHandler(async (req, res) => {
        ...
        if (!['owner', 'admin'].includes(member.role))
            throw new AppError(ErrorCode.AUTH_INSUFFICIENT_PERMISSION);
        const r = await db.query(`DELETE FROM workspace_invites WHERE id = $1 AND workspace_id = $2`, [id, workspaceId]);
        if (r.rowCount === 0)
            throw new AppError(ErrorCode.WORKSPACE_INVITE_NOT_FOUND);
        res.status(204).end();
    }));
    return router;
}
// Mounted at: /invites
export function inviteAcceptRouter(db) {
    const router = Router();
    ...
    router.post('/accept', requireAuth, asyncHandler(async (req, res) => {
        ...
        const inviteR = await db.query(`SELECT id, workspace_id, email, role, token, invited_by, accepted_at, expires_at, created_at
         FROM workspace_invites
        ...
        if (invite.accepted_at !== null)
            throw new AppError(ErrorCode.WORKSPACE_INVITE_ALREADY_ACCEPTED);
        if (new Date() > invite.expires_at)
            throw new AppError(ErrorCode.WORKSPACE_INVITE_EXPIRED);
        ...
        try {
           ...
           VALUES ($1, $2, $3)
           ON CONFLICT (workspace_id, user_id) DO NOTHING`, [invite.workspace_id, req.auth.userId, invite.role]);
        ...
        const workspaceR = await db.query(`SELECT id, name, slug, owner_id, logo_url, created_at FROM workspaces WHERE id = $1`, [invite.workspace_id]);
...
//# sourceMappingURL=invites.handler.js.map