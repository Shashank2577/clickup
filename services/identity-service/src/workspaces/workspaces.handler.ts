...
function toMemberDto(row: { workspace_id: string; user_id: string; role: string; joined_at: Date }) {
...
export function workspacesRoutes(db: Pool): Router {
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (member.role === 'owner') throw new AppError(ErrorCode.WORKSPACE_OWNER_CANNOT_LEAVE)
