...
import { FoldersRepository, FolderRow, FolderWithLists } from './folders.repository.js'

function toFolderDto(row: FolderRow) {
  return {
    id: row.id,
...
function toFolderWithListsDto(row: FolderWithLists) {
  return {
    ...toFolderDto(row),
...
export function spaceFoldersRoutes(db: Pool): Router {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const { name, color, isPrivate } = req.body as { name?: string; color?: string; isPrivate?: boolean }
...
export function foldersRoutes(db: Pool): Router {
  const router = Router()
  const repository = new FoldersRepository(db)
  ...
  router.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!folderId) throw new AppError(ErrorCode.VALIDATION_INVALID_INPUT, 'folderId is required')

      const folder = await repository.getFolder(folderId)
      if (!folder) throw new AppError(ErrorCode.FOLDER_NOT_FOUND)
