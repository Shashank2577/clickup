...
function toDto(row: SavedSearchRow) {
  return {
    id: row.id,
    userId: row.user_id,
...
export function savedSearchesRouter(db: Pool): Router {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const r = await db.query<SavedSearchRow>(
        `SELECT id, user_id, workspace_id, name, query, filters, created_at
         FROM saved_searches
    ...
    asyncHandler(async (req, res) => {
      ...
      const r = await db.query<SavedSearchRow>(
        `INSERT INTO saved_searches (user_id, workspace_id, name, query, filters)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, user_id, workspace_id, name, query, filters, created_at`,
        [req.auth.userId, workspaceId, input.name, input.query, JSON.stringify(input.filters)],
      )
  ...
  router.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (input.query !== undefined) { setClauses.push(`query = $${idx++}`); values.push(input.query) }
      ...
      if (r.rowCount === 0) throw new AppError(ErrorCode.SAVED_SEARCH_NOT_FOUND)
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const r = await db.query(
        `DELETE FROM saved_searches WHERE id = $1 AND user_id = $2 AND workspace_id = $3`,
