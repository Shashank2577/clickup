...
async function buildDigestBody(notifications: NotificationRow[], userName: string): Promise<string> {
  const lines: string[] = [
    `Hi ${userName},`,
    '',
...
export async function runDigestJob(db: Pool): Promise<void> {
  try {
    ...
    const { rows: digestTargets } = await db.query<DigestRow>(`
      SELECT
        u.id         AS user_id,
        u.email,
        u.name,
    ...
    for (const target of digestTargets) {
      try {
        ...
        const { rows: notifs } = await db.query<NotificationRow>(
          `SELECT type, payload, created_at FROM notifications
           WHERE user_id = $1
             AND is_read = FALSE
        ...
        await emailService.sendNotificationEmail(
          target.email,
          `Your ClickUp digest: ${target.unread_count} notification${target.unread_count !== 1 ? 's' : ''}`,
          body,
        )
...
export function startDigestRunner(db: Pool): void {
