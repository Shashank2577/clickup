...
import { TASK_EVENTS, COMMENT_EVENTS, NOTIFICATION_EVENTS, NotificationType, } from '@clickup/contracts';
import { createNotificationRepository } from './notifications.repository.js';
import { EmailService } from '../email/email.service.js';
export async function startNotificationSubscribers(db) {
    const repository = createNotificationRepository(db);
    const emailService = new EmailService();
    await subscribe(TASK_EVENTS.ASSIGNED, async (payload) => {
        ...
        logger.info({ taskId: payload.taskId, assigneeId: payload.assigneeId }, 'notification created: task_assigned');
    }, { durable: 'notif-svc-task-assigned' });
    await subscribe(COMMENT_EVENTS.CREATED, async (payload) => {
        ...
        for (const userId of payload.mentionedUserIds) {
            ...
            logger.info({ taskId: payload.taskId, mentionedUserId: userId }, 'notification created: task_mentioned');
        }
    }, { durable: 'notif-svc-comment-created' });
    // Dedicated mention event — carries authorId, sends email via EmailService
    await subscribe(COMMENT_EVENTS.MENTIONED, async (payload) => {
        if (!payload.mentionedUserIds || payload.mentionedUserIds.length === 0)
            return;
        for (const userId of payload.mentionedUserIds) {
            ...
            await emailService.sendMentionEmail(`${userId}@placeholder.local`, 'A teammate', `Task ${payload.taskId}`, `https://app.clickup-oss.local/tasks/${payload.taskId}?comment=${payload.commentId}`);
            logger.info({ commentId: payload.commentId, mentionedUserId: userId }, 'mention email dispatched');
        }
    }, { durable: 'notif-svc-comment-mentioned' });
    // Automation-triggered notifications — published by automations-service
    await subscribe(NOTIFICATION_EVENTS.SEND, async (payload) => {
        await repository.createNotification({
            userId: payload.userId,
            type: payload.type,
            payload: payload.payload,
        });
        logger.info({ userId: payload.userId, type: payload.type }, 'notification created: automation triggered');
    }, { durable: 'notif-svc-notification-send' });
}
//# sourceMappingURL=notifications.subscriber.js.map