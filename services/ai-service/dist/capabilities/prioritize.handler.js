...
import { PrioritizeInputSchema, PrioritizeOutputSchema, ErrorCode, } from '@clickup/contracts';
...
export function createPrioritizeRouter() {
    const router = Router();
    router.post('/prioritize', requireAuth, asyncHandler(async (req, res, _next) => {
        const input = validate(PrioritizeInputSchema, req.body);
        const messages = [{ role: 'user', content: buildPrioritizePrompt(input) }];
        ...
        if (!idsMatch) {
            logger.error({ inputIds: [...inputIds], outputIds: [...outputIds] }, 'Claude returned mismatched task IDs for prioritize');
...
//# sourceMappingURL=prioritize.handler.js.map