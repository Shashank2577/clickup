 ...
 */
export async function checkAiRateLimit(workspaceId) {
    ...
    if (requestCount > MAX_REQUESTS_PER_MIN) {
        throw new AppError(ErrorCode.AI_RATE_LIMITED, `AI request limit of ${MAX_REQUESTS_PER_MIN}/min exceeded for this workspace.`);
    }
}
/**
 ...
 */
export async function recordTokenUsage(workspaceId, inputTokens, outputTokens) {
    const redis = getRedis();
