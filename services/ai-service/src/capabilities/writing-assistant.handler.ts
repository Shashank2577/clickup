...
type WritingAssistantAction = z.infer<typeof WritingAssistantInputSchema>['action']

function buildPrompt(text: string, action: WritingAssistantAction, targetLanguage?: string): string {
  switch (action) {
    case 'improve':
...
export function createWritingAssistantRouter(): Router {
  const router = Router()
