...
type DocType = z.infer<typeof DocGenerationInputSchema>['docType']

interface DocGenerationOutput {
  title:   string
  content: string
  outline: string[]
}
...
export function createDocGenerationRouter(): Router {
  const router = Router()
  ...
  router.post(
    ...
    asyncHandler(async (req, res, _next: NextFunction) => {
      ...
      let parsed: DocGenerationOutput
      try {
        // Claude sometimes wraps JSON in markdown code fences — strip them
        const raw = response.content.replace(/^```(?:json)?\n?/m, '').replace(/\n?```$/m, '').trim()
        parsed = JSON.parse(raw) as DocGenerationOutput
