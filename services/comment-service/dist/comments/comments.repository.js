...
async function createComment(db, input) {
    const { rows } = await db.query('INSERT INTO comments (task_id, user_id, content, parent_id) VALUES ($1, $2, $3, $4) RETURNING *', [input.taskId, input.userId, input.content, input.parentId]);
    return rows[0];
}
